...
class JackTokenizer {
	...
	public function tokenize() {
		...
		foreach( str_split( $this->file ) as $char ) {
			...
			$index = array_search( $char, str_split( $this->file ) );
			$this->currentToken = str_split( $this->file )[$index ];
			if ( isset( str_split( $this->file )[$index + 1] ) ) {
				$this->nextToken = str_split( $this->file )[$index + 1];
			}
			...
			if ( in_array( $token, $this->keywords ) &&  ( $this->nextToken == ' ' || $this->nextToken == "(" ) ) {
				$this->writeToken( 'keyword', $token );
				$token = '';
				continue;
			}
			...
			if ( in_array( $char, $this->symbols ) ) {
				if ( strlen( $token ) > 1 ) {
					...
					} elseif ( in_array( $prevToken, $this->keywords ) ) {
						$this->writeToken( 'keyword', $prevToken );
					} else {
